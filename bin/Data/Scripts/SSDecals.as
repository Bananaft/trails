...
void MoveCamera(float timeStep)
{
    ...
    if (ui.cursor.visible && input.mouseButtonDown[MOUSEB_LEFT])
        PaintDecal();
}
...
void PaintDecal()
{
    ...
    if (Raycast(250.0f, hitPos, hitDrawable))
    {
        ...
        Node@ decalNode = scene_.CreateChild("decal");
        decalNode.position = hitPos;
        decalNode.rotation = Quaternion(0.0f, Random(360.0f), 0.0f);
        decalNode.SetScale(0.1f);
        StaticModel@ decalObject = decalNode.CreateComponent("StaticModel");
        decalObject.model = cache.GetResource("Model", "Models/Box.mdl");
        decalObject.material = cache.GetResource("Material", "Materials/Stone.xml");
        decalObject.castShadows = true;
    }
}
